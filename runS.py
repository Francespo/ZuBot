...
def generate_detection_dict(image: np.ndarray, tolerance: float) -> dict[str, bool]:
    ...
    detection_dict = {}
    # Run template matching sequentially to reduce peak CPU load.
    for template_name in templates.keys():
        detection_dict[template_name], _ = evaluate_template(template_name)
    return detection_dict
def next_iteration(img):
...
try:
    with mss.mss() as sct:
        # Take an initial screenshot to compare against.
        last_img = take_screenshot(sct)
        i = 0
        while True:
            current_img = take_screenshot(sct)
            # Only run detection if the screen has changed significantly.
            if are_images_different(last_img, current_img):
                print(f"Screen changed. Starting iteration number: {i}")
                next_iteration(current_img)
                i += 1
            last_img = current_img
            time.sleep(random.uniform(1.5, 2.0)) # Increased sleep time
